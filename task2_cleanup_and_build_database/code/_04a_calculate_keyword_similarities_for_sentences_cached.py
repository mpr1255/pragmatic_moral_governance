...
"""
Smart cached version of keyword similarity calculations for sentences.
...
SENTENCES_CACHE_DIR = CACHE_DIR / "sentences"
HASH_FILE = CACHE_DIR / "sentence_category_hashes.json"

# Create directories
SENTENCES_CACHE_DIR.mkdir(parents=True, exist_ok=True)
...
def calculate_category_similarities(sentences_df, centroid, category_name):
    """Calculate similarities for a single category."""
    cosine_sims = []
    euclidean_dists = []
    
    # Convert embeddings to numpy arrays
    sentence_embeddings = sentences_df['embedding'].apply(
        lambda x: np.array(eval(x)) if isinstance(x, str) else np.array(x)
    ).tolist()
    
    # Calculate similarities
    for sentence_embedding in tqdm(sentence_embeddings, 
                                desc=f"Processing {category_name} - sentences",
                                leave=True):
        cosine_sim = cosine_similarity_numba(sentence_embedding, centroid)
        euclidean_dist = euclidean_distance_numba(sentence_embedding, centroid)
        cosine_sims.append(cosine_sim)
        euclidean_dists.append(euclidean_dist)
    
    # Create results dataframe with minimal columns for caching
...
def main():
    print("Smart keyword similarity calculation for sentences with caching...")
    
    # Load data
    print("Loading sentence embeddings...")
    sentences_df = pd.read_csv(f"{OUTPUT_FOLDER}/section2_sentences_embeddings.csv")
    ...
    for category in categories:
        ...
        if changed:
            ...
            cache_file = SENTENCES_CACHE_DIR / f"{category}_similarities.csv"
    ...
    if categories_to_process:
        ...
        for category in tqdm(categories_to_process, desc="Processing categories"):
            ...
            # Calculate similarities
            results_df = calculate_category_similarities(sentences_df, centroid, category)
    ...
    # Start with base sentence data
    combined_df = sentences_df.copy()
    ...
    # Save combined results with consistent naming
    ...
    old_output_file = f"{OUTPUT_FOLDER}/section2_sentence_embeddings_with_cosine_similarity_to_keywords.csv"
    combined_df.to_csv(old_output_file, index=False)
    print(f"Also saved to (for compatibility): {old_output_file}")

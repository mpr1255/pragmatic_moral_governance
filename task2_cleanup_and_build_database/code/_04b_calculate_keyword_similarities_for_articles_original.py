...
def calculate_average_similarities(args):
    article_embedding, category_keywords = args
    cosine_similarities = []
    euclidean_distances = []
    for _, keyword_row in category_keywords.iterrows():
        ...
        if article_embedding.shape[0] != keyword_embedding.shape[0]:
            raise ValueError("Mismatched dimensions between article and keyword embeddings.")
        cosine_similarity = cosine_similarity_numba(article_embedding, keyword_embedding)
        euclidean_distance = euclidean_distance_numba(article_embedding, keyword_embedding)
...
def add_category_similarity_columns(df: pd.DataFrame, category: str, categories_keywords_embeddings: str) -> pd.DataFrame:
    ...
    article_embeddings = [(np.array(eval(row['embedding'])) if isinstance(row['embedding'], str) else np.array(row['embedding']), category_keywords) for index, row in df.iterrows()]

    with Pool(cpu_count()) as pool:
        similarities = list(tqdm(pool.imap(calculate_average_similarities, article_embeddings), total=len(article_embeddings), desc=f"Processing {category} - articles"))
...
def main():
    # Check if article embeddings exist
    article_embeddings_file = f'{OUTPUT_FOLDER}/section2_articles_embeddings.csv'
    if not os.path.exists(article_embeddings_file):
        print(f"Article embeddings file not found: {article_embeddings_file}")
        print("Please run article embedding generation first.")
        return
    
    print("Loading article embeddings...")
    article_embeddings = pl.read_csv(article_embeddings_file).to_pandas()
    ...
    for category in tqdm(categories, desc="Processing categories"):
        ...
        article_embeddings = add_category_similarity_columns(article_embeddings, category, categories_keywords_embeddings)

    output_file = f'{OUTPUT_FOLDER}/section2_article_embeddings_with_similarity_to_keywords_cosine_and_euclidean.csv'
    article_embeddings.to_csv(output_file, index=False)
    print(f"Saved article keyword similarities to: {output_file}")
